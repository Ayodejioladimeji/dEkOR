...
const getDashboardMetrics = async (
  ...
  try {
    ...
    const allOrders = await Order.find({ user: user?.id });
    ...
    const userAddress = await Address.countDocuments({ user: user?.id });
    ...
    const transactions = await Transaction.countDocuments({ user: user?.id });
